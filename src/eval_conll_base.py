...
def tokenize_and_align(ex, tokenizer, max_length: int):
    tok = tokenizer(ex["tokens"], is_split_into_words=True, truncation=True, max_length=max_length)
...
def main():
    ...
    ap.add_argument("--model_id", type=str, required=True, help="HF model id or local folder")
    ...
    # Force fast tokenizer + from_slow to avoid tokenizer.json issues
    tokenizer = AutoTokenizer.from_pretrained(args.model_id, use_fast=True, from_slow=True)
    ...
    print(f"\n=== CoNLL2003 {args.split.upper()} METRICS (BASE) ===")
