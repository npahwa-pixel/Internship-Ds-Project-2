...
def enable_gradient_checkpointing(model) -> None:
    # non-reentrant is safer on some backends
    try:
...
def freeze_bottom_layers(model, n_freeze: int) -> None:
    """Freeze embeddings + bottom N transformer layers (DeBERTa v3 friendly)."""
    if n_freeze <= 0:
        return
    ...
    if base is None:
        print("WARN: Could not locate base DeBERTa module; skipping freezing.")
        return
    ...
    if layers is None:
        print("WARN: Could not locate encoder layers; skipping freezing.")
        return
